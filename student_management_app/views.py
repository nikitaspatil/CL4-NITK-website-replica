...
from .models import CustomUser, Staffs, Students, AdminHOD,Courses
...
def StudentCouncil(request):
	return render(request, 'StudentCouncil.html')
...
def doRegistration(request):
	...
	if CustomUser.objects.filter(username=username).exists():
		...
		return render(request, 'registration.html')
	print(username,email_id,password,user_type,first_name,last_name)
	user = CustomUser()
	...
	if user_type == CustomUser.STAFF:
		...
		Students.objects.create(admin=user,course_id=data)
